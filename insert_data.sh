...
do
  ...
  then
    ...
    if [[ -z $WINNER_ID ]]
    then
      echo "TEAM ID '$WINNER' NOT FOUND, creating new ID"
    ...
    # opponent not in teams
    if [[ -z $OPPONENT_ID ]]
    then
      echo "TEAM ID '$OPPONENT' NOT FOUND, creating new ID"
    ...
    echo "Added the following: $YEAR $ROUND $WINNER $OPPONENT $WINNER_GOALS $OPPONENT_GOALS"
  fi
done
